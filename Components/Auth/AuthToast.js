        ...
        {step === "OTP" && (
          <>
            ...
            <div className={`${styles.otpWrapper} ${shake ? styles.shake : ""}`}>
              <OtpInput
                value={otp}
                onChange={(v) => { if(/^\d*$/.test(v)) { setOtp(v); setOtpError(""); } }}
                numInputs={6}
                shouldAutoFocus
                inputType="tel"
                renderSeparator={<span style={{width:'8px'}} />}
                renderInput={(props) => <input {...props} className={`${styles.otpInput} ${otpError ? styles.otpErrorInput : ""}`} />}
              />
            </div>
            ...
            <button className={styles.submit} onClick={submitOtp} disabled={verifyOtpMutation.isPending}>
              {verifyOtpMutation.isPending ? "در حال بررسی..." : "ورود به تورینو"}
            </button>
          </>
        )}
      </div>
